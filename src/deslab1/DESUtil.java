...
/**
 * This class is used to contain functions related to DES algorithm
 *
 */
class DESUtil {
    /**************************************************************************/
    ...
    //s boxes for s box substitution
    ...
    //encrypt - encrypt the message with given key
    public static String encrypt(String message, int key){
        //declare variables
        String binSeq = "";
        char [] ch = new char[message.length()];
        message.getChars(0, message.length(), ch, 0);
        String [] binPTBlocks;
        String [] binCTBlocks;
        
        //convert message to binary sequence
        for(int i = 0; i < message.length(); i++){
            int tempChar = (int)ch[i];
            binSeq += Integer.toString(tempChar, 2);
        }
        //add padding
        int remainder = binSeq.length()%64;
        if(remainder != 0){
            String padding = "";
            for(int i = 0; i < (BLOCK_LENGTH - remainder); i++){
                padding += "0";
            }
            binSeq = padding + binSeq;
        }
        //break the binary sequence into blocks
        binPTBlocks = new String[binSeq.length()/BLOCK_LENGTH];
        int seqIndex = 0;
        for(int i = 0; i < binPTBlocks.length; i++){
            binPTBlocks[i] = binSeq.substring(seqIndex, seqIndex+64);
        ...
        for (int i = 0; i < binPTBlocks.length; i++){
            binCTBlocks[i] = encryptBlock(binPTBlocks[i]);
        }
        
        //construct the ciphertext binary
        binSeq  = "";
        for(String s: binCTBlocks){
            binSeq += s;
        }
